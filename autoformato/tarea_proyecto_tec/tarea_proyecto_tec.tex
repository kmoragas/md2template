...
\coordinate [below=5.15cm,left=-4.5cm] (midpoint) at (current page.north);
...
$if(Fondo)$
\node[anchor=north west,inner sep=0pt] at (0,-2.3) {\includegraphics[width=\paperwidth]{$Fondo$}}; % Background image
$else$
\node[anchor=north west,inner sep=0pt] at (0,-2.3) {\includegraphics[width=\paperwidth]{figures/background.jpg}}; % Background image
$endif$
...
\draw[anchor=north] (midpoint) node [fill=bannergray,fill opacity=0.8,text opacity=1,inner sep=1cm]{
	...
	\Huge\bfseries\sffamily\parbox[c][][t]{\paperwidth}{
			...
			{\Huge \fontspec{Fira Sans} \color{bannerred} \so { $Titulo$}	}\\[20pt] % Subtitle
